...
@section('conteudo')
    <div class="col-sm-12">
        <div class="card">
            ...
            <form class="form-material" action="{{ route('administracao.rh.perfil.index') }}" method="GET" role="">
    ...
    <div class="col-sm-12">
        <div class="card">
                                    ...
                                    <td class="text-center col-md-2">
                                        <div class="text-center table-actions">
                                            @can('acesso', 'administracao.rh.perfil.edit')
                                                <a type="button" class="btn  btn-sm btn-primary" title="Editar perfil"
                                                    href="{{ route('administracao.rh.perfil.edit', [$idx->id]) }}">
                                                    <i class="ti-pencil"></i>Editar
                                                </a>
                                            @endcan
                                            @can('acesso', 'administracao.rh.perfil.destroy')
                                                <a type="button" class="btn  btn-sm btn-danger" title="Excluir Perfil"
                                                ...
                                                <form id="{{ $idx->id }}"
                                                    action="{{ route('administracao.rh.perfil.destroy', [$idx->id]) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
            ...
            <div class="card-footer">
                {{ $listPerfil->links('pagination::bootstrap-4') }}
            </div>
        </div>
    </div>
@endsection
