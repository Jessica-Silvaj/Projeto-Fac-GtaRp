...
@section('conteudo')
    <div class="col-sm-12">
        <div class="card">
            ...
            <form class="form-material" action="{{ route('bau.lancamentos.historico') }}" method="GET" role="">
                <div class="card-block">
                    <div class="form-row align-items-end">
                        <div class="form-group form-default form-static-label col-md-3 mb-3">
                            ...
                            <label for="inicio" class="float-label">Início</label>
                        </div>
                        <div class="form-group form-default form-static-label col-md-3 mb-3">
                        ...
                        <div class="form-group form-default form-static-label col-md-4 mb-3">
                            <select name="itens_id" id="itens_id" class="form-control select2"
                                ...
                                @if (!empty($itemSelecionado))
                                    <option value="{{ $itemSelecionado['id'] }}" selected>{{ $itemSelecionado['nome'] }}</option>
                                @endif
                            </select>
                            <span class="form-bar"></span>
                        ...
                        <div class="form-group form-default form-static-label col-md-2 mb-3">
                            <select name="granularidade" id="granularidade" class="form-control select2" placeholder="Exibição">
                    ...
                    <div class="form-row align-items-center mt-2">
                        <div class="form-group col-md-12 mb-0 metric-options">
                            <label class="d-inline-block mr-3 mb-1 metric-title">Métrica</label>
                            <label class="form-check form-check-inline mb-0 mr-3">
                                <input class="form-check-input" type="radio" name="modo" id="modo_quantidade" value="quantidade" {{ ($modo ?? 'quantidade') === 'quantidade' ? 'checked' : '' }}>
                                <span class="form-check-label">Quantidade</span>
                            </label>
                            <label class="form-check form-check-inline mb-0">
                                <input class="form-check-input" type="radio" name="modo" id="modo_movimentos" value="movimentos" {{ ($modo ?? '') === 'movimentos' ? 'checked' : '' }}>
        ...
        <div class="card">
            <div class="card-header d-flex align-items-center justify-content-between">
                ...
                <div class="text-right export-actions">
                    <a class="btn btn-sm btn-outline-secondary btn-export" href="{{ route('bau.lancamentos.historico.csv', array_merge(request()->all(), ['dataset' => 'serie'])) }}">Série CSV</a>
                    <a class="btn btn-sm btn-outline-secondary btn-export" href="{{ route('bau.lancamentos.historico.csv', array_merge(request()->all(), ['dataset' => 'top_entradas'])) }}">Top Entradas CSV</a>
                    <a class="btn btn-sm btn-outline-secondary btn-export" href="{{ route('bau.lancamentos.historico.csv', array_merge(request()->all(), ['dataset' => 'top_saidas'])) }}">Top Saídas CSV</a>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="col-md-4">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <h2 class="mb-0 {{ $liquido >= 0 ? 'text-success' : 'text-danger' }}">{{ number_format($liquido, 0, ',', '.') }}</h2>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="col-sm-12">
        <div class="row">
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header d-flex align-items-center justify-content-between">
                        <h3 class="mb-0">Entradas x Saídas por {{ ($granularidade ?? 'dia')==='semana' ? 'semana' : (($granularidade ?? 'dia')==='mes' ? 'mês' : 'dia') }} ({{ ($modo ?? 'quantidade') === 'movimentos' ? 'Movimentações' : 'Quantidade' }})</h3>
                    </div>
                    <div class="card-body">
            ...
            <div class="col-md-4" id="col-right-historico">
                <div class="card mb-3">
                    <div class="card-header d-flex align-items-center justify-content-between">
                        <h3 class="mb-0">Top Itens (Entradas — {{ ($modo ?? 'quantidade') === 'movimentos' ? 'Movimentações' : 'Quantidade' }})</h3>
                    </div>
                    <div class="card-body">
                ...
                <div class="card">
                    <div class="card-header d-flex align-items-center justify-content-between">
                        <h3 class="mb-0">Top Itens (Saídas — {{ ($modo ?? 'quantidade') === 'movimentos' ? 'Movimentações' : 'Quantidade' }})</h3>
                    </div>
                    <div class="card-body">
        ...
        (function () {
            ...
            function renderLegend(containerId, data, colors) {
                ...
                data.forEach(function (d, i) {
                    ...
                    tx.textContent = d.label + ' (' + d.value + ')';
                    item.appendChild(sw);
                    item.appendChild(tx);
                    el.appendChild(item);
                });
            ...
            function drawBarAligned() {
                ...
                if (lastCard) {
                    ...
                    h = Math.max(220, Math.min(1200, h - 8));
                    cont.style.height = h + 'px';
                }
                if (mainChart) { cont.innerHTML = ''; mainChart = null; }
                ...
                setTimeout(function(){
                    ...
                    rects.forEach(function(r){ r.setAttribute('rx', 3); r.setAttribute('ry', 3); });
                }, 0);
            }

            var donutE = null, donutS = null; 
            var showingAllE = false, showingAllS = false;
            var colorsE = ['#2ca02c','#98df8a','#1f77b4','#aec7e8','#ff7f0e','#ffbb78','#d62728','#ff9896','#9467bd','#c5b0d5'];
            var colorsS = ['#d62728','#ff9896','#1f77b4','#aec7e8','#2ca02c','#98df8a','#ff7f0e','#ffbb78','#9467bd','#c5b0d5'];
            ...
            setTimeout(function(){ buildDonutE(); buildDonutS(); drawBarAligned(); }, 50);
            var resizeTimer;
            window.addEventListener('resize', function(){
                clearTimeout(resizeTimer);
                resizeTimer = setTimeout(function(){ buildDonutE(); buildDonutS(); drawBarAligned(); }, 120);
            });
        })();
    </script>
@endsection
