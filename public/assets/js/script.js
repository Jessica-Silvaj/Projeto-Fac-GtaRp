...
$(document).ready(function() {
        ...
        $('.monetario-mask').mask('#.##0,00', {
            allowZerofor: false,
            placeholder: "0,00",
            reverse: true
        });

        $('.decimal4-mask').mask('#.##0,0000', {
            allowZerofor: false,
            placeholder: "0,0000",
            reverse: true
        });

        $('.decimal6-mask').mask('#.##0,000000', {
            allowZerofor: false,
            placeholder: "0,000000",
            reverse: true
        });

        $('.decimal8-mask').mask('#.##0,00000000', {
            allowZerofor: false,
...
function mostrarObrigatoriedade(form, mostrar) {
    const rules = form.validate().settings.rules;

    for (const key in rules) {
        const requiredSpan = $(`span[id="required-${key}"]`);

        if (mostrar && rules[key]) {
            requiredSpan.remove();
            $(`label[for="${key}"]`).append(`<span id="required-${key}" class="text-danger">*</span>`);
        } else {
            requiredSpan.remove();
        }
    }
...
function configurarValidacao(form, rules, messages) {
    return form.validate({
        ...
        errorPlacement: function(error, element) {
            ...
            element.closest('.form-row > .form-group').append(error);
        },
...
function validarFormulario(form, rules, messages) {
    const formA = configurarValidacao(form, rules, messages);
    mostrarObrigatoriedade(form, true);
